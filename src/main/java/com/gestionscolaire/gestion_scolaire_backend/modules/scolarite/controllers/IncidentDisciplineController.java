...
public class IncidentDisciplineController {
    ...
    public ResponseEntity<IncidentDiscipline> enregistrer(@Valid @RequestBody IncidentDisciplineRequest request) {
        IncidentDiscipline incident = IncidentDiscipline.builder()
                ...
                .build();
        IncidentDiscipline saved = incidentDisciplineService.enregistrerIncident(
    ...
    public ResponseEntity<List<IncidentDiscipline>> listerParEleve(@PathVariable Long eleveId) {
        return ResponseEntity.ok(incidentDisciplineService.listerParEleve(eleveId));
    }

    @GetMapping("/classe/{classeId}")
    @PreAuthorize("hasAnyRole('SUPER_ADMIN', 'DIRECTEUR', 'SECRETAIRE', 'SURVEILLANT_GENERAL')")
    public ResponseEntity<List<IncidentDiscipline>> listerParClasseEtDate(
    ...
    @PreAuthorize("hasAnyRole('SUPER_ADMIN', 'DIRECTEUR', 'SURVEILLANT_GENERAL')")
    public ResponseEntity<IncidentDiscipline> marquerTraite(@PathVariable Long id, @RequestBody Map<String, String> body) {
